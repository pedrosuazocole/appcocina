...
def cargar_datos():
    ...
    cols_rec = ["Plato", "Detalle_Receta", "Costo_Total_Plato", "Precio_Venta", "Valor_Utilidad", "Margen_Utilidad", "Margen_Objetivo"]
    ...
    if os.path.exists(FILE_NAME):
        try:
            ...
            for col in cols_rec:
                if col not in rec.columns: rec[col] = 0.0
            
            if 'Codigo' in art.columns: art['Codigo'] = art['Codigo'].astype(str).str.zfill(3)
...
if 'catalogo' not in st.session_state:
    st.session_state.catalogo, st.session_state.ingredientes, st.session_state.recetas, st.session_state.historial = cargar_datos()
...
def guardar():
    try:
        ...
        st.error(f"No se pudo guardar. Cierra el Excel si está abierto. Error: {e}")

# 2. MENÚ LATERAL
st.sidebar.header("MENÚ PRINCIPAL")
opciones_menu = {
    "cat": "Lista de Artículos (Maestro)", 
...
if opid == "cat":
    t1, t2 = st.tabs(["🆕 Registrar", "✏️ Modificar"])
    with t1:
        with st.form("f_cat", clear_on_submit=True):
            ...
            if st.form_submit_button("Guardar"):
                if n_art:
                    ...
                    st.session_state.catalogo = pd.concat([st.session_state.catalogo, pd.DataFrame([{"Codigo": nuevo_cod, "Nombre": n_art, "Unidad": u_art}])], ignore_index=True)
                    guardar(); st.success(f"Creado: {nuevo_cod}"); st.rerun()
    with t2:
        if not st.session_state.catalogo.empty:
            art_m = st.selectbox("Seleccione artículo", [f"{r['Codigo']} - {r['Nombre']}" for _, r in st.session_state.catalogo.iterrows()])
            c_m = art_m.split(" - ")[0]
            ...
            with st.form("f_edit_c"):
                ...
                n_u = st.selectbox("Nueva Unidad", ["Unidad", "Libra", "Kg", "Litro", "Onza"], index=["Unidad", "Libra", "Kg", "Litro", "Onza"].index(st.session_state.catalogo.at[idx, 'Unidad']))
                if st.form_submit_button("Actualizar"):
                    st.session_state.catalogo.at[idx, 'Nombre'], st.session_state.catalogo.at[idx, 'Unidad'] = n_n, n_u
                    guardar(); st.success("Actualizado"); st.rerun()
    st.dataframe(st.session_state.catalogo, use_container_width=True, hide_index=True)

elif opid == "inv":
    # --- 1. MOTOR DE CÁLCULO (Stock Real y Costos) ---
    if not st.session_state.historial.empty:
        df_ent = st.session_state.historial.copy()
        df_ent['Codigo'] = df_ent['Codigo'].astype(str).str.zfill(3)
        df_ent[['Cantidad', 'Costo_Total']] = df_ent[['Cantidad', 'Costo_Total']].apply(pd.to_numeric)
        resumen_ent = df_ent.groupby(['Codigo', 'Producto', 'Unidad']).agg({'Cantidad': 'sum', 'Costo_Total': 'sum'}).reset_index()
        
        if 'historial_prod' in st.session_state and not st.session_state.historial_prod.empty:
            # Reconstruir salidas por ingrediente desde el historial de producción
            salidas_por_codigo = {}
            for _, fila_prod in st.session_state.historial_prod.iterrows():
                try:
                    import ast as _ast
                    insumos = _ast.literal_eval(fila_prod['Detalle'])
                    for ins in insumos:
                        cod = str(ins['Codigo']).zfill(3)
                        total_sal = ins['Cantidad'] * fila_prod['Cantidad']
                        salidas_por_codigo[cod] = salidas_por_codigo.get(cod, 0) + total_sal
                except:
                    pass
            resumen_ent['Codigo'] = resumen_ent['Codigo'].astype(str).str.zfill(3)
            resumen_ent['Salidas'] = resumen_ent['Codigo'].map(salidas_por_codigo).fillna(0)
            resumen_ent['Stock'] = resumen_ent['Cantidad'] - resumen_ent['Salidas']
            resumen_ent['Costo_Unitario'] = resumen_ent['Costo_Total'] / resumen_ent['Cantidad']
            st.session_state.ingredientes = resumen_ent[['Codigo', 'Producto', 'Unidad', 'Stock', 'Costo_Unitario']].rename(columns={'Producto': 'Ingrediente'})
        else:
            resumen_ent['Stock'] = resumen_ent['Cantidad']
            resumen_ent['Costo_Unitario'] = resumen_ent['Costo_Total'] / resumen_ent['Cantidad']
            st.session_state.ingredientes = resumen_ent[['Codigo', 'Producto', 'Unidad', 'Stock', 'Costo_Unitario']].rename(columns={'Producto': 'Ingrediente'})
    else:
        st.session_state.ingredientes = pd.DataFrame(columns=['Codigo', 'Ingrediente', 'Unidad', 'Stock', 'Costo_Unitario'])

    tab1, tab2, tab3 = st.tabs(["➕ Registrar/Editar Compra", "⚙️ Gestionar Historial/Inventario", "📜 Kardex Detallado"])

    with tab1:
        st.markdown("### 📝 Datos de la Factura")
        c_f1, c_f2 = st.columns(2)
        f_fecha = c_f1.date_input("Fecha de Emisión", key="f_fecha_reg")
        f_num = c_f2.text_input("No. Factura", placeholder="Ej: FAC-123")

        op_p = ["SELECCIONE PRODUCTO"] + [f"{r['Codigo']} - {r['Nombre']}" for _, r in st.session_state.catalogo.iterrows()]
        p_sel = st.selectbox("Buscar Insumo", op_p)
        
        u_act, nom_p, cod_s = "---", "", ""
        if p_sel != "SELECCIONE PRODUCTO":
            cod_s = p_sel.split(" - ")[0].zfill(3)
            df_cat = st.session_state.catalogo[st.session_state.catalogo["Codigo"].astype(str).str.zfill(3) == cod_s]
            if not df_cat.empty:
                u_act, nom_p = df_cat.iloc[0]['Unidad'], df_cat.iloc[0]['Nombre']
        ...
        if st.button("➕ Agregar Producto a Detalle"):
            if p_sel != "SELECCIONE PRODUCTO" and p_cant > 0 and f_num:
                ...
                st.rerun()
            else: st.warning("Complete No. Factura, Producto y Cantidad.")
        ...
        if st.session_state.factura_temporal:
            ...
            if col_b1.button("💾 PROCESAR FACTURA E INVENTARIO", type="primary", use_container_width=True):
                st.session_state.historial = st.session_state.historial[st.session_state.historial['No_Factura'] != f_num]
                for it in st.session_state.factura_temporal:
                    nueva_f = pd.DataFrame([{**it, "Fecha_Factura": f_fecha, "No_Factura": f_num}])
                    st.session_state.historial = pd.concat([st.session_state.historial, nueva_f], ignore_index=True)
                guardar(); st.session_state.factura_temporal = []
                st.success(f"✅ ¡Éxito! La Factura {f_num} ha sido agregada al inventario correctamente."); st.rerun()
            
            if col_b2.button("🗑️ Cancelar Factura", use_container_width=True):
                st.session_state.factura_temporal = []; st.rerun()

    with tab2:
        st.markdown("### 📊 Inventario Físico Real")
        if not st.session_state.ingredientes.empty:
            ...
            st.dataframe(df_vis.rename(columns={'Costo_Unitario': 'Costo Prom.'}), hide_index=True, use_container_width=True)
            st.info(f"💰 **Valor Total de Inversión en Bodega: L. {df_vis['Valor_Total'].sum():.2f}**")
        ...
        if not st.session_state.historial.empty:
            ...
            for n_f, grp in df_hist.groupby("No_Factura"):
                with st.expander(f"📄 Factura: {n_f} | Fecha: {grp.iloc[0]['Fecha_Factura']} | Total: L. {grp['Costo_Total'].sum():.2f}"):
                    st.dataframe(grp[['Codigo', 'Producto', 'Cantidad', 'Costo_Unitario', 'Costo_Total']], hide_index=True)
                    c_ed1, c_ed2, c_ed3 = st.columns([0.3, 0.4, 0.3])
                    
                    if c_ed1.button(f"✏️ Modificar", key=f"mod_{n_f}"):
                        st.session_state.factura_temporal = grp[['Codigo', 'Producto', 'Unidad', 'Cantidad', 'Costo_Unitario', 'Costo_Total']].to_dict('records')
                        st.info("Cargado en 'Registrar'. Haga sus cambios y guarde.")
                    
                    # Sistema de advertencia antes de eliminar
                    ...
                    if c_ed3.button(f"❌ ELIMINAR", key=f"del_{n_f}", type="secondary", disabled=not confirmar):
                        st.session_state.historial = st.session_state.historial[st.session_state.historial['No_Factura'] != n_f]
                        guardar(); st.rerun()

    with tab3:
        st.subheader("📜 Kardex Detallado por Producto")
        insumo_k = st.selectbox("Seleccione Producto:", op_p, key="k_p_sel")
        if insumo_k != "SELECCIONE PRODUCTO":
            c_k = insumo_k.split(" - ")[0].zfill(3)
            m_ent = st.session_state.historial[st.session_state.historial['Codigo'].astype(str).str.zfill(3) == c_k].copy()
            m_sal = pd.DataFrame()
            if 'historial_prod' in st.session_state and not st.session_state.historial_prod.empty:
                # Reconstruir salidas por ingrediente desde historial_prod
                sal_rows = []
                for _, fila_prod in st.session_state.historial_prod.iterrows():
                    try:
                        import ast as _ast
                        ...
                        for ins in insumos:
                            if str(ins['Codigo']).zfill(3) == c_k:
                                sal_rows.append({
                                    ...
                                    'Cantidad': ins['Cantidad'] * fila_prod['Cantidad'],
                                    'Costo_Unitario': ins.get('Costo_U', 0)
                                })
                    except:
                        pass
                if sal_rows:
            ...
            for _, r in m_ent.iterrows():
                k_list.append({'Fecha': r['Fecha_Factura'], 'Ref': f"Fact: {r['No_Factura']}", 'E': r['Cantidad'], 'S': 0.0, 'Total': r['Costo_Total']})
            for _, r in m_sal.iterrows():
                k_list.append({'Fecha': r['Fecha'], 'Ref': r['Ref'], 'E': 0.0, 'S': r['Cantidad'], 'Total': -(r['Cantidad'] * r.get('Costo_Unitario', 0))})

            if k_list:
                df_k = pd.DataFrame(k_list).sort_values('Fecha')
                stock_a, valor_a, filas = 0.0, 0.0, []
                for _, m in df_k.iterrows():
                    stock_a += (m['E'] - m['S'])
                    valor_a += m['Total']
                    filas.append({'Fecha': m['Fecha'], 'Ref': m['Ref'], 'U': u_act, 'Entrada': m['E'], 'Salida': m['S'], 'Existencia': stock_a, 'C. Prom': valor_a/stock_a if stock_a > 0 else 0, 'V. Total': valor_a})
    ...
    with t_crear:
        st.subheader("Configuración de Plato")
        ...
        if not st.session_state.ingredientes.empty:
            opciones_i = {f"{r['Codigo']} - {r['Ingrediente']}": r for _, r in st.session_state.ingredientes.iterrows()}
            # Filtrar defaults para que solo incluya opciones que existen actualmente
            val_insumos_validos = [v for v in val_insumos if v in opciones_i]
            insumos_sel = st.multiselect("Seleccione los productos:", list(opciones_i.keys()), default=val_insumos_validos)
            ...
            if insumos_sel:
                for s in insumos_sel:
                    ...
                    if st.session_state.edit_rec_data:
                        try:
                            ...
                            for ie in items_e:
                                if str(ie['Codigo']).zfill(3) == str(datos['Codigo']).zfill(3): val_cant = float(ie['Cantidad'])
                        except: pass

                    col1, col2, col3, col4 = st.columns(4)
                    with col1: st.markdown(f"**{datos['Ingrediente']}**")
                    with col2: cant_rec = st.number_input(f"Cant ({datos['Unidad']})", min_value=0.01, step=0.1, value=val_cant, key=f"q_{datos['Codigo']}")
                    ...
                    detalle_final.append({"Codigo": datos['Codigo'], "Nombre": datos['Ingrediente'], "Cantidad": cant_rec, "Costo_U": datos['Costo_Unitario'], "Subtotal": subt})
                ...
                if p_venta > 0:
                    if porc_utilidad < m_objetivo: st.error(f"⚠️ Margen bajo el objetivo.")
                    else: st.success(f"✅ Margen óptimo.")
                ...
                if st.button("💾 GUARDAR RECETA", type="primary", use_container_width=True):
                    if n_plato and p_venta > 0 and detalle_final:
                        if not st.session_state.recetas.empty:
                            st.session_state.recetas = st.session_state.recetas[st.session_state.recetas['Plato'] != n_plato]
                        nueva_rec = pd.DataFrame([{"Plato": n_plato, "Detalle_Receta": str(detalle_final), "Costo_Total_Plato": costo_materia_prima, "Precio_Venta": p_venta, "Valor_Utilidad": val_utilidad, "Margen_Utilidad": porc_utilidad, "Margen_Objetivo": m_objetivo}])
                        st.session_state.recetas = pd.concat([st.session_state.recetas, nueva_rec], ignore_index=True)
                        guardar(); st.session_state.edit_rec_data = None; st.success("Receta guardada"); st.rerun()
        
        if st.session_state.edit_rec_data:
            if st.button("❌ Cancelar Edición"):
                st.session_state.edit_rec_data = None; st.rerun()

    with t_ver:
        if not st.session_state.recetas.empty:
            for idx, r in st.session_state.recetas.iterrows():
                with st.expander(f"🍲 {r['Plato']} | L.{r['Precio_Venta']}"):
                    # 1. Tabla de Detalle
                    import ast
                    items = ast.literal_eval(r['Detalle_Receta'])
                    st.table(pd.DataFrame(items)[['Nombre', 'Cantidad', 'Costo_U', 'Subtotal']])
                    
                    # 2. LÍNEA DE TOTALIZACIÓN DETALLADA (En una sola fila)
                    st.markdown(f"""
                    **RESUMEN:** Precio Venta: **L.{r['Precio_Venta']:.2f}** | 
                    ...
                    # BOTÓN EDITAR CON MENSAJE
                    if c1.button(f"📝 Editar {r['Plato']}", key=f"btn_edit_{idx}"):
                        st.session_state.edit_rec_data = r.to_dict()
                        st.success(f"✅ Datos de '{r['Plato']}' cargados. Por favor, regrese a la pestaña 'Crear / Editar Receta'.")
                        # No hacemos rerun inmediato para que el usuario lea el mensaje
                    ...
                    if not st.session_state[f"confirm_del_rec_{idx}"]:
                        if c2.button(f"🗑️ Eliminar {r['Plato']}", key=f"btn_pre_del_{idx}"):
                            st.session_state[f"confirm_del_rec_{idx}"] = True; st.rerun()
                    else:
                        ...
                        if col_si.button("✅ Confirmar", key=f"conf_si_rec_{idx}"):
                            ...
                            guardar(); st.session_state[f"confirm_del_rec_{idx}"] = False; st.rerun()
                        if col_no.button("❌ Cancelar", key=f"conf_no_rec_{idx}"):
                            st.session_state[f"confirm_del_rec_{idx}"] = False; st.rerun()
        else: st.info("No hay recetas registradas.")
elif opid == "prod":
    t_orden, t_hist_prod = st.tabs(["🚀 Generar Orden de Producción", "📜 Historial de Producción"])
    
    # Asegurar que el historial de producción exista en el estado de la sesión
    if 'historial_prod' not in st.session_state:
        # Intentamos cargar desde una hoja llamada 'Produccion' si existe en tu Excel
        try:
            st.session_state.historial_prod = pd.read_excel(FILE_NAME, sheet_name='Produccion')
            # Eliminar IDs duplicados que pudieron quedar por el bug anterior
            st.session_state.historial_prod = st.session_state.historial_prod.drop_duplicates(subset='ID', keep='last').reset_index(drop=True)
        except:
    ...
    with t_orden:
        ...
        if st.session_state.recetas.empty:
            st.warning("No hay recetas creadas. Vaya al módulo 'Crear Receta' primero.")
        else:
            with st.container(border=True):
            ...
            if plato_p:
                ...
                for ins in insumos_receta:
                    total_necesario = ins['Cantidad'] * cant_p
                    # Obtener Stock real (Sincronizado)
                    idx_inv = st.session_state.ingredientes[st.session_state.ingredientes['Codigo'] == ins['Codigo']].index
                    stock_actual = st.session_state.ingredientes.at[idx_inv[0], 'Stock'] if not idx_inv.empty else 0
                    
                    if stock_actual < total_necesario: puede_procesar = False
                ...
                if st.button("🔥 PROCESAR PRODUCCIÓN", type="primary", disabled=not puede_procesar, use_container_width=True):
                    id_prod = f"PROD-{datetime.now().strftime('%H%M%S%f')[:13]}"
                    fecha_hoy = datetime.now().strftime("%Y-%m-%d %H:%M")

                    # 1. ACTUALIZAR STOCK EN TABLA MAESTRA
                    ...
                    for ins in insumos_receta:
                        ...
                        idx = st.session_state.ingredientes[st.session_state.ingredientes['Codigo'] == ins['Codigo']].index
                        if not idx.empty:
                            st.session_state.ingredientes.at[idx[0], 'Stock'] -= total_n

                    # 2. REGISTRAR EN EL HISTORIAL DE PRODUCCIÓN
                    ...
                    st.session_state.historial_prod = pd.concat([st.session_state.historial_prod, nueva_fila_prod], ignore_index=True)
                    
                    # 3. GUARDAR TODO EN EL EXCEL (Incluyendo la nueva hoja de producción)
                    try:
                        with pd.ExcelWriter(FILE_NAME) as writer:
                            st.session_state.catalogo.to_excel(writer, sheet_name='Catalogo', index=False)
                            st.session_state.ingredientes.to_excel(writer, sheet_name='Inventario', index=False)
                        ...
                        st.success(f"Producción registrada correctamente. ID: {id_prod}")
                        st.balloons()
                        st.rerun()
                    except Exception as e:
                        st.error(f"Error al guardar: Cierre el archivo Excel. {e}")
    ...
    with t_hist_prod:
        ...
        if not st.session_state.historial_prod.empty:
            ...
            df_h = st.session_state.historial_prod
            if busq: df_h = df_h[df_h['Plato'].str.contains(busq)]
            
            for idx, row in df_h.iterrows():
                with st.expander(f"📦 {row['Fecha']} | {row['Plato']} | Cant: {row['Cantidad']}"):
                    st.write(f"ID Operación: {row['ID']}")
                    
                    # Lógica de eliminación con reversión
                    if st.button(f"🗑️ Eliminar y Revertir Stock", key=f"del_p_{row['ID']}"):
                        import ast
                        ins_rev = ast.literal_eval(row['Detalle'])
                        # Devolver los ingredientes al stock maestro
                        for i_r in ins_rev:
                            total_r = i_r['Cantidad'] * row['Cantidad']
                            idx_inv = st.session_state.ingredientes[st.session_state.ingredientes['Codigo'] == i_r['Codigo']].index
                            if not idx_inv.empty:
                                st.session_state.ingredientes.at[idx_inv[0], 'Stock'] += total_r
                        
                        # Eliminar del historial de producción
                        st.session_state.historial_prod = st.session_state.historial_prod.drop(idx)
                        ...
                        st.warning("Producción eliminada y stock restaurado.")
                        st.rerun()
        else:
            st.info("No hay registros de producción.")
elif opid == "rep":
    st.subheader("📊 Panel de Control y Análisis")

    # 1. MÉTRICAS GENERALES
    c1, c2, c3 = st.columns(3)
    with c1:
        total_inv = (st.session_state.ingredientes['Stock'] * st.session_state.ingredientes['Costo_Unitario']).sum() if not st.session_state.ingredientes.empty else 0
        st.metric("Inversión en Bodega", f"L. {total_inv:,.2f}")
    with c2:
        st.metric("Recetas Activas", len(st.session_state.recetas))
    with c3:
        margen_prom = st.session_state.recetas['Margen_Utilidad'].mean() * 100 if not st.session_state.recetas.empty else 0
    ...
    with col_izq:
        ...
        bajo_stock = st.session_state.ingredientes[st.session_state.ingredientes['Stock'] < 5].copy()
        ...
        # --- BOTÓN DE DESCARGA (Ahora siempre visible debajo de las alertas) ---
        if not st.session_state.ingredientes.empty:
            import io
            buf_rep = io.BytesIO()
            # Descargamos el reporte de stock bajo si existe, sino el inventario completo
            df_descarga = bajo_stock if not bajo_stock.empty else st.session_state.ingredientes[['Ingrediente', 'Stock', 'Unidad']]
    ...
    with col_der:
        st.write("### 💰 Top 5 Platos más Rentables")
        if not st.session_state.recetas.empty:
    ...
    if not st.session_state.recetas.empty:
        with st.expander("📖 Ver Detalle de Costeo"):
            df_rep_rec = st.session_state.recetas[['Plato', 'Costo_Total_Plato', 'Precio_Venta', 'Valor_Utilidad', 'Margen_Utilidad']].copy()
            df_rep_rec['Margen %'] = (df_rep_rec['Margen_Utilidad'] * 100).round(2)
            st.dataframe(df_rep_rec[['Plato', 'Costo_Total_Plato', 'Precio_Venta', 'Valor_Utilidad', 'Margen %']], hide_index=True, use_container_width=True)